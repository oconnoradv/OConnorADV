...
<html><head><meta charset="UTF-8"></head><body><p>import { clsx } from "clsx";
...
const ButtonInput = ({
  ...
  if (link) {
    return (
      </a><a href="https://www.oconnoradv.com/wp-content/plugins/updraftplus/vendor/team-updraft/lib-onboarding-wizard/Wizard/Onboarding/src/components/Inputs/link" target='{"_blank"}' classname="{classes}">
        {children}
      </a>
    );
  }
  ...
  return (
    <button type="{props.type" onclick="{onClick}" classname="{classes}" disabled>
      {children}
    </button>
  );
};
...
export default ButtonInput;</button></htmlbuttonelement></htmlbuttonelement></p></body></html>
